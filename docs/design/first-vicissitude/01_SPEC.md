...
| 二阶段可见（羽片隐藏） | 3,044 |
| 关节与锚点 | 65 |
| 图集 | 256×256（263 个区域，占用 48,576 像素） |
...
- **下身**：`LOWER_ROOT → SPINE_TAIL_1…3`，无腿。`CLOTH_FRAGMENT_*` 是祭衣残幅，
  由下腹两侧向后包绕、中间敞开，每幅横向两个折面、纵向三段、共 12 三角面，左右长度不同；
  不新增上下摆关节或布料物理，褶皱主要画在纹理上。
...
单张 256×256 图集加同 UV 的自发光层 `first_vicissitude_emissive.png`
