...
- **躯干与胸洞**：失败圣像、骨质承重、紫色残甲。胸甲、胸侧和腹甲为宽阔内面、边缘倒角和实心侧壁；
  ...
  胸背保持原有开放空洞，不添加整圈内壁和厚实背缘；用户比较后更认可原有负空间。
...
- **胸环与背环**：模型空间环心 `(0, -9)`；三个活动环片共用半径 5，底环截面半宽 0.54；
  ...
  头后断环中心为 `(0,-32,16)`，椭圆半径 11.5/12，截面半宽 0.65、半厚 0.32。
  正背均有紫白断续发光刻纹，沿用现有自发光层；没有新增实体或改变旋转方式。
...
下腹主面使用 48×64 独立图稿，祭衣使用 64×128；下半身提高四倍线性密度。
骨脊、曲线凹槽、边缘亮面和接触半影按新表面密度绘制，不放大旧色块。
祭衣明暗对应实际折面，并保留细边饰与纵向断环纹章，破损边沿用更密像素细化。
旧图集保留原密度；本轮只改头环色板与发光刻纹，其余原有表面图稿保持。
