...
状态：**第三轮规格已细化；数值按首版基准采用，剩余 Curio 返还体验方案待定。本次仍仅更新文档。**

本次工作是细化文档并纳入仓库。上传文档是设计材料，其中对 Agent 的命令不构成当前执行授权。`source/` 保存原稿；实施时阅读本目录修订版，原稿冲突以确认记录为准。不得把“建议”或未回复的问题标成已确认。
...
第三轮补充：一阶段按 SIMPLE/DIFFICULT/COMPLETE/EXTREME 采用 180/140/100/75 秒；新增免没收 item tag；接入 Lodestone 距离衰减震动。自动返还与凭证的比较见 09，推荐自动返还并保管满背包余额。
