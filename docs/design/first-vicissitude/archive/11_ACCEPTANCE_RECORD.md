...
- 难度时长（03/D9）：SIMPLE/DIFFICULT/COMPLETE/EXTREME = 1800/1400/1000/750；首次有效攻击锁定；旧档按 `PhaseOneTicks/6000` 比例换算到新时长；`PhaseTwoStarted=true` 映射 PHASE_TWO。
...
- 震动（08）：只在三个一次性事件上触发，全部走 VicissitudeEffectPacket 并由客户端按
  key 去重：一阶段转场第 1 tick（16 tick / 0.25）、重击命中（12 tick / 0.30，
  第十三轮由 8 tick / 0.15 加强）、
  死亡 36–59「翼根失效」（12 tick / 0.18）。全强度距离 8 格、最大 24 格，客户端倍率
  irstVicissitude.screenshakeIntensity 0–1（0 关闭）。死亡第 60 tick 的核心熄灭只做弱光收束，
  不再补一次震动；不随每个球的命中震动。
- 特效（10）：翼尖蓄光、释放碎芒、地面预警法阵粒子（黑色符文 + 冷白边界微光，含 60° 缺口）、斩击 builder（横斩用 `setSlashAngle`、竖劈用 `setVerticalSlashAngle`）、死亡熄灭粒子、位置震动（重击 12/0.30、转场 16/0.25、死亡 12/0.18，全强度 8 格、最大 24 格，客户端 0–1 倍率）。
...
35. **羽翼乱翻（第十轮次要动态的实现错误）**：`dragWingsYaw/Pitch` 的实参已经是**角度**，
    ...
    现在改为取身体转角的 **60%** 并封顶 **±30°**（`WING_DRAG_FRACTION` / `WING_DRAG_CAP`），
    翼根 / 外翼 / 羽片按 1.0 / 0.6 / 0.4 递减，峰值分别不超过 30° / 18° / 12°。
    新增离线检查 `RigMeshCheck#checkWingComposure`：扫 3060 组动作/阶段/收翼/时钟组合，
    逐关节逐轴比对"作者姿态包络 + 约 15%"的上限（实测峰值：翼根 10.6/61.5/56.4、外翼 7.3/49.0/18.2、
    下翼 0/46.0/0、羽片 5.2/12.0/0），任何再次乘错倍数都会当场断言失败。
...
37. **地面预兆重做为法阵粒子，删除几何环**：原预警是世界空间里用 `RenderType.lightning()` 画的
    分段四边形环。那是原版闪电/天气用的渲染类型（加色、全亮、写天气目标），所以无论传
    `COLD_WHITE` / `COLD_BONE` / `DARK_VIOLET` 都会显成发白发黄的一圈，与黑紫冷色主题不符。
    现在几何绘制整段删除（`renderGroundMarkers` / `renderGroundMarker` / `renderRing` /
    `flatQuad` / `insideGap` / `GROUND_SEGMENTS`，以及 `MaledictClientRenderEvents#onRenderLevel`
    和它的 `RenderLevelStageEvent` 订阅），改由每客户端 tick 的符文粒子承担：胸口标记是随收束边
    长大的整圆符文（判定就是整个圆盘，画满圆不说谎），环裁定是 12 个固定槽位的断环符文
    （每 tick 复活一枚、寿命 14、半径分三档），槽位角度与符文尺寸都按缺口内缩
    `atan2(scale, radius)`，中心与环外不放任何符文。边界另有冷白微光，在「收束边」与「判定外沿」
    之间交替。全部 `enableForcedSpawn()`，最低粒子档位下仍然显示——这替代了原来「几何保证低粒子
    可读」的职责。数值抽查（内 3 外 6、缺口 60°、全进度全槽位）：符文覆盖半径 3.06–5.94 严格落在
    环带内，与缺口边缘的余量 ≥ 0°。

38. **法阵颜色改为纯黑**：符文用 `SIGIL_BLACK`（#000000）。能成立的关键是渲染类型仍是
    `LUMITRANSPARENT`：着色器先用**贴图自身亮度**替换贴图 alpha、再乘顶点色（rgb 相乘、a 相乘），
    所以白色笔画保持不透明、黑底透明，而 RGB 乘黑即得到黑色符文。Malum 侵蚀权杖用
    `MALIGNANT_BLACK` 画近黑圆环是同一手法。注意 Malum 的符文贴图是**全不透明**的白字黑底
    （实测 alpha 恒为 255），因此不能改用普通 `TRANSPARENT`——那会拍出一整块黑方块。边界微光
    保持冷白，作为黑符文在暗色地面上的对比。

39. **重击震动加强**：按用户要求把重击命中的屏幕震动加大。`EVENT_HEAVY_IMPACT` 从
    8 tick / 峰值 0.15 改为 **12 tick / 峰值 0.30**（转场 16/0.25、死亡 12/0.18 不动，
    客户端 0–1 倍率照旧生效）。08 与 10 的基准行、本记录的对应条目已同步。

**命令**：`.\gradlew compileJava --offline` 与 `.\gradlew test --offline` BUILD SUCCESSFUL
（测试 5 项 0 失败）。
**未验证**：法阵黑白对比在日夜与不同地表上的实际观感、最低粒子档位下是否仍读得出判定边界、
重击 0.30 的强度是否合适（Lodestone 强度单位为观感校准，未做游戏内确认）。

## 2026-09-20：行为节奏与参考面数预算

本轮修改见 [13](13_COMBAT_AND_STYLE_REFINEMENT.md)，只读参考统计见 [14](14_REFERENCE_GEOMETRY_BUDGET.md)。

- 起手固定目标、后段锁定朝向，移除命中帧瞬间转身；施法及收势制动，飞镰离手时侧移；返回恢复不覆盖正在进行的动作。
- 修正齐射重复末波、冲刺状态未重置、投镰起手清零冷却、冷却等待消耗近战轮换的问题。
- 最终主体为 **1,488 三角面 / 108 网格 / 65 关节与锚点**。原 23,244 面减少约 93.6%，14,100 面中间稿已被替代。资产与预览全部重新生成。
- 最终 Gradle 8.8 / Java 17：`compileJava test --offline`，**BUILD SUCCESSFUL**；5 个测试类共 28 个测试，0 失败、0 错误。既有测试不覆盖完整 Minecraft 战斗场景。
- Blender 验证通过：12/12 胸洞射线贯通、头核正面遮挡、胸肩体积、贴图对比、UV 与法线。
- `RigMeshCheck`：84,480 次实际网格顶点检查及 3,060 次翼部姿态采样通过；胸环反向旋转和主环平面检查通过。
- 没有改动 `src/generated`；本轮未运行 `runData`，因为没有数据生成器变更。
- 实机战斗手感、多人/专服行为和性能未测，不将离线渲染或面数降低当成实机验收。

## 2026-09-20：减面后翼部反馈修订

见 [15 骨翼支撑与羽面重做](15_WING_STRUCTURE_AND_FEATHERS.md)。在 1,488 面稿上更换翼部几何和材质，最终 128 网格 / 1,792 三角面，65 个关节与锚点不变。

- 每侧八枚扇形主羽、五枚内侧次羽、正面八枚/背面四枚覆羽；实心骨翼具有开放分叉、关节与放射骨指。
- 新增四格原创羽毛 cutout，保留浅折脊和前后错层，右翼局部露骨；复用现有双面 cutout 渲染。
- `compileJava --offline`：BUILD SUCCESSFUL。只改资产与导出几何，本轮未重复运行与本次外观无关的 JUnit 测试。
- Blender 的顶点、法线、UV、胸洞、头核、色板与四格羽毛透明/实色验证通过；`RigMeshCheck` 的 159,744 次网格检查、3,060 次翼部姿态检查及环旋转检查通过。
- 重新生成 `.blend`、`.bbmodel`、运行时网格、两张纹理、翼部包围盒和预览。`src/generated` 没有改动。
- 模型结构参考来自三个模组的公开模型声明，未拷贝外部代码或资源；实机动画、光照及碰撞手感尚未验收。

## 2026-09-20：历史对照后的身体重做

见 [17 历史对照与肢体力量感重做](17_BODY_MASS_AND_HISTORY_REVIEW.md)。该阶段交付为 168 网格 / 3,312 三角面 / 65 关节与锚点，前述低面数版本保留为历史记录。

- Git 对照早期胸部特写及建模源，恢复环绕肋骨、背侧肩胛、前臂双骨、腕环、残躯横突和环刻轨；扩大肩胸与护臂体块，保留新版羽翼。
- Blender 顶点/法线/UV、12/12 胸洞射线、周围胸腹实体、头核遮挡与羽毛 cutout 检查通过。
- `RigMeshCheck`：159,744 次翼网格检查、3,060 次翼部姿态检查及胸环/主环旋转检查通过。这些检查不等同于手臂全动作穿模检测。
- 人工查看了本轮胸部特写、两阶段主视角和二阶段侧视图；实体肩臂、腕部连接与胸背厚度可辨。预览为离线渲染，未作实机验收。
- 同步导出 Blender、Blockbench、游戏网格、纹理及预览。本轮不改战斗代码或 `src/generated`，未重复运行无关的 JUnit 测试。

## 2026-09-20：肘腕与手部修订

- 当前交付为 186 网格 / 3,572 三角面 / 65 关节与锚点，新增肘轴、两侧护片、腕块、掌体、回扣指节与拇指侧轮廓。
- 根据握拳读感的反馈，采用共用掌体、三组凸出指节与下方回扣指尖，拇指压在内侧。各指块刚性绑定手关节，手部与持械锚点保持不变。
...
## 2026-09-21：骨架承重与同心残环

- 最新交付见 [18](18_OSSUARY_AND_CONCENTRIC_RINGS.md)：217 网格 / 5,246 三角面 / 65 关节。胸肩、肱骨和前臂改为粗骨承托与局部残甲；胸洞、胸环共用从运行时导出的圆心。
- 胸环与背环增加实体截面、银轨和横向连接件，保留原有阶段及断裂方向。
- 12 条中央射线与新增 32 条环周射线全部贯通；两阶段完整常态旋转中，23,736 次实际胸环顶点检查通过，半径保持约 4.46–5.54。
- 网格法线、UV、头核、材质、159,744 次翼顶点检查、3,060 次翼姿态和环旋转检查通过；两阶段正侧背、胸部与手部特写、灰模和夜间预览已更新。
- 检查针对离线资产与骨架；完整实机施法、持械和性能仍属于下列未验证项。未改动 `src/generated`。

## 大块明暗与转场落羽修订

- 图集重排尝试为 16×16 单元，145 个区域占用 199 个单元；其单元对齐、区域边界和透明缺口技术检查通过。但用户已否决视觉结果：仍是少量模板重复，未正确拆分同一部件的不同表面。该版本没有通过贴图视觉验收；后续方案见 [21](21_SURFACE_UV_AND_TEXTURE_PLAN.md)，左右臂及左右翼优先镜像复用。

- 外壳减去部分倒角层，贴图改用宽亮面、侧影及下缘暗部；保留已确认的胸环。
- 新增分叉骨翼、错时落羽、翻滚下落与末段缩小；二阶段及其死亡不再渲染羽片。阶段完成标记同步客户端。
- 完整资产 215 部件 / 5,552 三角面，二阶段可见 4,952 面；同步 Blender、Blockbench、运行时网格和贴图。
- `compileJava test --offline` 通过；落羽时序和骨翼碰撞切换有单元测试。实际翼网格检查 160,128 次、胸环检查 23,736 次、翼姿态检查 3,060 次通过；胸洞与洞缘射线全部通过。
...
- 已更新 Blender、Blockbench、运行时模型和 256×256 贴图：肩臂、拳、肩胛、翼根及最高主翼的 50 个网格采用逐表面 UV，25 对左右部件共用对应岛。
- 图稿源为 `surface_sample.py`；侧面与端面分开，管状表面沿中心线长度展开。图集共 223 个区域、占用 55,488 像素；区域数量仅用于核对布局，不代表视觉质量。
- 本轮资产检查通过：UV 逐面边界、区域不相交、左右共享区域、运行时 UV 与 Blender 一致、法线、胸洞和胸环。面数仍为 5,552。
- 新近图为 `surface_sample_arm.png`、`surface_sample_arm_unlit.png`、`surface_sample_wing.png`；均为离线预览。其余身体与主翼仍是旧占位，没有标为全身重绘完成或视觉验收通过。

### 待验证场景

2026-09-23 躯干连接审查：同关节完整覆盖检查未找到直接可删面，改为简化六个支撑网格的截面，共减少 80 面。当前 185 网格 / 3,504 三角面，二阶段 3,204 面；见 [24](24_TORSO_SUPPORT_SIMPLIFICATION.md)。

2026-09-23 环装饰贴图替代：移除 30 个装饰网格，胸环、环座与背环保留 11 段实体，每段六类独立 UV 表面。资产为 185 网格 / 3,584 三角面，二阶段 3,284 面。逐面 UV、装饰几何移除、胸洞、同心半径及姿态检查通过，新增正背面和旋转离线近图；实现见 [23](23_PAINTED_RINGS.md)。

几何审查第一轮：总面数 5,552 → 4,416，二阶段 4,952 → 4,116。去掉环截面倒角和羽片折脊，以 0.30 模型单位偏差约束合并骨翼截面；详见 [几何预算审查](22_GEOMETRY_BUDGET_AUDIT.md)。这不是最终预算，也未声称实机帧率提升。

第二批表面贴图已扩展到躯干和完整骨翼：105 个网格、44 对镜像复用，图集占用 54,896 像素，仍为 256×256 与 5,552 三角面。逐面 UV、区域占用、镜像复用、导出一致性、法线、胸洞和胸环检查通过。新增躯干正背面、无灯光和二阶段整体离线预览；不据此声称完成实机视觉验收。
...
祭衣重做后续轮：仅左右两片改变，窄挂片改为宽幅包绕残衣，16×32 原创透明破边图稿；共增加 12 面至 3,344，资源检查通过。七组对照覆盖更新，实机布摆运动观感待验，见 [28](28_WRAPPED_VESTMENT.md)。

胸环、祭衣残片和晶体头壳修订：185 网格 / 3,332 面，二阶段 3,032 面；祭衣薄片减少 12 面，头壳调整尖角并重画晶面，胸环刻纹与腹部骨片细化。资源与 Java 动作检查通过，实机视觉待验，见 [27](27_CHEST_VESTMENT_AND_CRYSTAL.md)。

肩臂、胸背、拳头及追加腹部细化完成：表面宽亮面、甲片压暗、腹部骨片与连接缝重画；几何和 UV 对照一致，3,344 面不变，资源检查通过。固定七组对照重渲染为本轮身体机位，游戏内观感未验证，见 [26](26_BODY_SURFACE_REFINEMENT.md)。

主翼纹理细化后续轮：四类主翼正背面、内侧与端面重画，185 部件几何和 UV 与本轮 before 一致，仍为 3,344 面。导出与纹理布局检查通过；固定七组前后图滚动覆盖，不增加历史对照集合。详见 [25 最新记录](25_THIN_SPURS_AND_TEXTURE_COMPENSATION.md)。

2026-09-23 短分叉减面与纹理补偿：185 网格 / 3,344 三角面，二阶段 3,044 面；十个分叉 320 → 160 面，原正面轮廓顶点保留，其余 175 部件导出数据一致。UV、胸洞、同心环、翼部动作包围盒检查通过，已保存同条件前后离线图；背面明暗和侧视厚度存在取舍，实机观感未验证，详见 [25](25_THIN_SPURS_AND_TEXTURE_COMPENSATION.md)。
